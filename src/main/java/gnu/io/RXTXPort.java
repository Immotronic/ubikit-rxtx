...
final public class RXTXPort extends SerialPort
{
	...
	*/
	...
	{
		if (debug_events) {
			z.report( "RXTXPort:sendEvent(" + event + ", " + state + ")");
			z.report( "RXTXPort:sendEvent: fd=" + fd + ", SPEventListener=" + SPEventListener + ", monThread=" + monThread + ")");
		}
	...
	*/
			...
			throws IOException
		{
			...
			if ( fd == 0 )
			{
				...
				if(debug) z.reportln("+++++++ IOException()\n");
				throw new IOException();
			}

			if( b==null )
			{
				if(debug) z.reportln("+++++++ NullPointerException()\n");
				if (debug_read)
			...
			{
				if(debug) z.reportln("+++++++ IndexOutOfBoundsException()\n");
				if (debug_read)
